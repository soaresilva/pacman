    ...
    document.addEventListener("keydown", e => {
      if (!this.stopGame) {
        if (e.keyCode === 39) {
          ...
          this.mouth = this.element.classList.toggle(
            "entity--pac--mouthClosed"
          );
          this.pacDirection = "right";
          if (
            this.Stage.collisionDetection(this.xPos + 1, this.yPos) === true
          ) {
            this.yPos = this.yPos;
          } else {
          ...
          if (
            this.Stage.collisionDetection(this.xPos, this.yPos - 1) === true
          ) {
            this.yPos = this.yPos;
          } else {
            //adding score when apple is eaten
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const stage = new Stage(12, 7);
  stage.mount(app);

  const stageOne = document.querySelector(".stage");
  const pacBoy = new Pacman(stage, 0, 0, "mouth", 0, 0, "1Pac");
